...
Load this reference only when inspecting or seeding local Lecturn state directly.
...
```bash
node apps/server/scripts/lecturn-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/lecturn-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/lecturn-seed.sql
```

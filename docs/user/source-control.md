...
Lecturn connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

Lecturn works with the platforms your team already uses:
...
- Lecturn can suggest titles and descriptions based on your commits
...
- When an agent finishes a turn on your thread's branch, Lecturn checks for a newly opened
...
- Command-click (Control-click on Windows and Linux) a pull request number in the sidebar to open it in your browser instead of in Lecturn
...
1. Install the GitHub CLI (version 2.81.0 or newer) on the machine running Lecturn:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Lecturn and verify GitHub shows as authenticated
...
machine running Lecturn.

Recommended, a Bitbucket access token:

```bash
export LECTURN_BITBUCKET_ACCESS_TOKEN="your-access-token"
```
...
```bash
export LECTURN_BITBUCKET_EMAIL="you@example.com"
export LECTURN_BITBUCKET_API_TOKEN="your-token"
```

If both are set, the access token wins. Restart Lecturn and verify the connection in **Source
Control settings**.
...
**Git is required** – Lecturn uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Lecturn (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.

**Common issues:**

- **Provider shows "Not authenticated"** – Run the login command for that provider (e.g., `gh auth login`) in a terminal on the server, then rescan in Settings
- **GitHub says it could not verify sign-in status** – Lecturn needs GitHub CLI 2.81.0 or newer to check sign-in status. Update `gh` (e.g., `brew upgrade gh`), then rescan

...
The same configuration lives in `~/.lecturn/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Lecturn writes the built-in defaults into that file on

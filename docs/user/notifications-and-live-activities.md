...
Use **Show activity notch** or **Hide activity notch** in Pull Requests or the **Lecturn activity** menu-bar menu to enable or hide it. On an internal display with a recognizable notch area, the panel joins the top edge around the camera housing, with compact controls beside the camera and expanded content extending downward. On other displays, or when that area cannot be identified, use **Show activity** from the menu bar. The celestial background and illuminated-book logo match Lecturn. The panel follows the app’s Light, Dark, or System appearance setting, including the compact header and changes while it is open. Light surfaces use copper edges with warm metallic highlights, keeping the thread sheen visible on cream backgrounds. CI indicators use deeper state colors for clarity. Peek and expanded views open with a short resize and content reveal; reduced motion keeps these transitions immediate. A travelling metallic border marks active work: gold in dark mode and copper in light mode. It stops when hidden or when reduced motion is enabled. Approval, input and error states keep explicit labels.
...
Activity states share the same visual cues across the Mac panel, web, and phone: a travelling gold (dark mode) or copper (light mode) sheen for active work, pulsing amber for attention, red for failure, green for completion, and grey for idle or offline work. Labels remain available in tooltips and accessibility descriptions. Motion pauses off screen and respects reduced motion. Passing checks alone do not mark a PR complete. On the phone, the sheen follows the rounded card outline with a bright crest and fading trail. Settled threads retain a red-gold border around the Unsettle action and use a light copper-tinted surface in light mode. Reduced motion keeps a static metallic highlight.

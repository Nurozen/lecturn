...
Lecturn runs Google's official Antigravity ACP agent. By default it signs in with your personal
...
not sign in this Lecturn provider.
...
2. Choose **Install Antigravity**. Lecturn downloads the official runtime from Google to that
...
6. Wait for Lecturn to confirm sign-in and load the model choices. Select an Antigravity model
...
machine as the Lecturn environment, sign-in can finish directly.
...
return URL field in Lecturn. Choose **Continue** on web or desktop, or **Complete sign-in** on
mobile. Do not change the address to your server's hostname.

Return to the same Lecturn client and environment where you started sign-in. Another client
...
a thread or bug report. Lecturn waits for Google's agent to confirm sign-in. A successful
...
are stored in plain text in Lecturn settings on that environment and are passed only to the
...
A nonempty **Binary path** takes priority. With the field empty, Lecturn uses its managed
runtime, then an installation on the environment's `PATH`. An invalid explicit path reports
an error instead of selecting another installation. Lecturn does not update or remove manual
...
Lecturn asks you to select an available model instead of silently changing it.
On mobile, a saved default model choice remains usable while the account advertises that
default. Its picker entry shows the current model's name and capabilities.
...
New nested folders are supported. File access stays within the workspace and attachment
directories; symbolic links to files outside those directories are rejected.

When the agent offers **Allow for this thread** on a shell or web tool, Lecturn shows Google's
...
Lecturn keeps thread history and file diffs. Antigravity does not support conversation rewind,
...
or reply ownership. Lecturn cannot show separate child entries or separate child replies
...
installation. Lecturn refuses removal while the runtime is in use. An explicit **Binary path**
must be cleared in web or desktop provider settings before that instance can install or remove
the managed runtime. These controls become available again after clearing the path. You can
still observe and cancel a shared installation while a custom path is configured.

## Account access and errors

Google controls account eligibility, models, and usage limits. Lecturn does not report your
...
Use any retry time Google supplies. Lecturn does not switch to an API key to get past the limit.

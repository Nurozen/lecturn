...
This guide is for people who want to use more than one Codex account in Lecturn. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Lecturn](./install.md).
...
issue. Lecturn uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
...
reports them. If Codex does not report either value, Lecturn leaves it out instead of using the
...
When a Codex tool needs access to an app such as Safari, Lecturn shows the app name and asks for
...
- both accounts can see the same Lecturn/Codex sessions
...
In Lecturn Settings, name it something obvious:
...
In Lecturn Settings, add another Codex provider:
...
Lecturn shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Lecturn treats it

...
machine running your Lecturn server does that, its themes appear in the theme library alongside
your own, and Lecturn follows them: change the desktop theme and Lecturn retints with it, without
a restart.
...
different theme in Settings afterwards sticks until the next `lecturn theme set`, and running the same
set again is how you bring clients back. `lecturn theme clear` removes the setting without changing
what anyone currently has, and `lecturn theme show` prints the current theme and everything the
machine publishes. Only the environment you are anchored to publishes themes, so a remote client
follows the machine it is connected to, not the device it runs on. Lecturn Mobile keeps its own
...
the standard Lecturn look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the Lecturn state
directory (`~/.lecturn/userdata/themes/` by default). The filename is the theme id — `nightfall.json`
...
exported from Lecturn (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. Lecturn generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a Lecturn theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of Lecturn does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so Lecturn never reads

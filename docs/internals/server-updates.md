...
The launcher is the only runtime writer of `service-state.json`. `lecturn service install` and
`lecturn service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `lecturn@<target>` into a unique staging directory.
...
```sh
npx lecturn@<version> service update
```

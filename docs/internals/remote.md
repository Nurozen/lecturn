...
Lecturn has one runtime boundary: a client talks to a Lecturn server over HTTP and WebSocket, and the server
...
└───────────────┬──────────────────────────────┘
                │ connects to one Lecturn server
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Lecturn server        │
...
One running Lecturn server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed Lecturn Connect relay tunnels.                                   |
...
Tailscale is the first provider, and Lecturn manages more than discovery. When `tailscaleServeEnabled` is
...
(`apps/server/src/server.ts`, using [`@lecturn/tailscale`](../../packages/tailscale/src/tailscale.ts)).
...
Access answers one question: how does the client speak WebSocket to a Lecturn server? It does not answer
how the server got started or who manages the process.

### Direct WebSocket access

`wss://lecturn.example.com` or `ws://10.0.0.15:3773`, paired as a bearer target. This is the base model.
...
Managed Lecturn Connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
Worker itself. See [lecturn-connect.md](./lecturn-connect.md).

### Tailscale access

A Lecturn-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote Lecturn
...
Launch answers a different question: how does a Lecturn server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs Lecturn and the client connects directly or through a
  tunnel.
...
- **Client-managed local publish.** A local server is published through the relay with
  `lecturn connect link`, exposing a desktop-hosted environment to mobile without router or firewall
  changes.
...
- a relay-hosted OAuth callback broker (see [lecturn-connect.md](./lecturn-connect.md));

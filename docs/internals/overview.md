...
The sweep looks a branch up from the thread's worktree when it still exists, so it shares the
per-cwd PR cache the sidebar polls instead of spending a second host request. Clients render the
persisted settlement state and do not derive settlement from PR or inactivity state. A committed `thread.settled` event also lets `ProviderCommandReactor` stop an idle provider
session.

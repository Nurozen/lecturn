...
When the reason for a decision will matter after implementation, record it in the relevant internal document. Write a separate decision record under [`docs/adr/`](../adr/README.md) only when the rationale does not fit cleanly beside the current architecture, for example when it breaks an existing convention.
...
`.plans/` and `artifacts/` are gitignored as a safety net for agent tools. Their presence does not make them accepted project artifacts. Pull requests must not add implementation plans or temporary research under another name.

...
## Verify the deployed Connect protocol

A green client build does not prove that the independently deployed relay accepts its protocol. Before promoting desktop or TestFlight builds after a protocol change:

1. Run the focused `EnvironmentLinker.test.ts` and `EnvironmentConnector.test.ts` suites under `infra/relay/src/environments`. Their signed wire fixtures deliberately pin public protocol identifiers independently of shared constants. Do not update these literals as a cosmetic rename: changing them requires a coordinated rollout.
2. Record the exact client build and Cloudflare Worker version serving traffic. Deploying the web app does not deploy the relay. Verify the relay deployment plan preserves existing database, tunnel, gateway and signing-key resources.
3. From that client build, sign in with a verified test account that has Connect access, publish an isolated test environment, list it in the web/mobile client, check its online status, and connect through the managed endpoint. This must exercise signed link, health and credential-mint requests against the deployed relay; `/health` alone is insufficient.
4. Publish agent activity and verify the intended mobile notification path. Record trace IDs without retaining bearer tokens, signed proofs or private keys in public logs.

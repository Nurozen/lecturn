...
Lecturn has one server-side observability model:
...
stdout/stderr at `~/.lecturn/ssh-launch/<state>/server.log`.
...
- SSH-managed launch persistence: `~/.lecturn/ssh-launch/<state>/server.log`
...
`<home>/userdata/logs/server.trace.ndjson` (so `~/.lecturn/userdata/...` by default, or
`/custom/path/userdata/...` with `--home-dir /custom/path`), a linked worktree dev run uses
`<worktree>/.lecturn/userdata/logs/server.trace.ndjson`, and an implicit dev run outside a linked
worktree uses `~/.lecturn/dev/logs/server.trace.ndjson`.
...
```bash
export LECTURN_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export LECTURN_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export LECTURN_OTLP_SERVICE_NAME=lecturn-local
```

Optional:

```bash
export LECTURN_TRACE_MIN_LEVEL=Info
export LECTURN_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `LECTURN_OTLP_*`.

macOS app bundle example:

```bash
LECTURN_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
LECTURN_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
LECTURN_OTLP_SERVICE_NAME=lecturn-desktop \
"/Applications/Lecturn.app/Contents/MacOS/Lecturn"
```
...
```bash
TRACE_FILE="${LECTURN_HOME:-$HOME/.lecturn}/userdata/logs/server.trace.ndjson"
```

A dev server started from a linked worktree defaults to that worktree's local home:

```bash
TRACE_FILE="$WORKTREE/.lecturn/userdata/logs/server.trace.ndjson"
```

Only an implicit dev run outside a linked worktree uses the shared dev directory:

```bash
TRACE_FILE="$HOME/.lecturn/dev/logs/server.trace.ndjson"
```
...
- service name such as `lecturn-local`, `lecturn-dev`, or `lecturn-desktop`
...
- `lecturn_rpc_request_duration`
- `lecturn_orchestration_command_duration`
- `lecturn_orchestration_command_ack_duration`
- `lecturn_provider_turn_duration`
- `lecturn_git_command_duration`

Counters tell you volume and failure rate:

- `lecturn_rpc_requests_total`
- `lecturn_orchestration_commands_total`
- `lecturn_provider_turns_total`
- `lecturn_git_commands_total`
...
`lecturn_orchestration_command_ack_duration` measures:
...
1. Check `lecturn_orchestration_command_ack_duration` by `commandType`.
...
- `LECTURN_OTLP_TRACES_URL` was not set
...
- `LECTURN_TRACE_FILE`: override trace file path
- `LECTURN_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `LECTURN_TRACE_MAX_FILES`: rotated file count, default `10`
- `LECTURN_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `LECTURN_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `LECTURN_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `LECTURN_OTLP_TRACES_URL`: OTLP trace endpoint
- `LECTURN_OTLP_METRICS_URL`: OTLP metric endpoint
- `LECTURN_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `LECTURN_OTLP_SERVICE_NAME`: service name, default `lecturn-server`

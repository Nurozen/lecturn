...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "lecturn-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-lecturn", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "lecturn", specifier: pluginPath }],

...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.LECTURN_HOME, path.resolve("/tmp/custom-t3"));
    ...
    it.effect("uses custom t3Home when provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.LECTURN_HOME, path.resolve("/tmp/my-t3"));
      }),
    );
  ...
  describe("runDevRunnerWithInput", () => {
    ...
    // Base-dir precedence (--home-dir > worktree .t3 > ambient LECTURN_HOME)
    ...
    describe("t3 home precedence", () => {
      ...
      it.effect("prefers the worktree .t3 over an ambient LECTURN_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({

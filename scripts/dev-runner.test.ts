...
const devServerInput = {
  mode: "dev:server",
  lecturnHome: "/tmp/lecturn-dev-runner",
  browser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit LECTURN_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "LECTURN_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "LECTURN_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.LECTURN_HOME, undefined);
        assert.equal(env.LECTURN_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.LECTURN_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          lecturnHome: "/tmp/custom-lecturn",
          browser: false,
        ...
        assert.equal(env.LECTURN_HOME, path.resolve("/tmp/custom-lecturn"));
        assert.equal(env.LECTURN_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.LECTURN_NO_BROWSER, "1");
        assert.equal(env.LECTURN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.LECTURN_LOG_WS_EVENTS, "1");
        assert.equal(env.LECTURN_HOST, "0.0.0.0");
    ...
    it.effect("strips inherited service-launcher context", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            LECTURN_SERVICE_LAUNCHER_CONTEXT: '{"childVersion":"9.9.9"}',
            LECTURN_BOOT_SERVICE_UNIT: "lecturn.service",
          },
          serverOffset: 0,
          webOffset: 0,
          lecturnHome: undefined,
        ...
        assert.equal(env.LECTURN_SERVICE_LAUNCHER_CONTEXT, undefined);
        assert.equal(env.LECTURN_BOOT_SERVICE_UNIT, undefined);
      }),
    );
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            LECTURN_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          lecturnHome: undefined,
        ...
        assert.equal(env.LECTURN_MODE, "web");
        assert.equal(env.LECTURN_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.LECTURN_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom lecturnHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.LECTURN_HOME, path.resolve("/tmp/my-lecturn"));
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            LECTURN_PORT: "13773",
            LECTURN_MODE: "web",
            LECTURN_NO_BROWSER: "0",
            LECTURN_HOST: "0.0.0.0",
        ...
        assert.equal(env.LECTURN_MODE, undefined);
        assert.equal(env.LECTURN_NO_BROWSER, undefined);
        assert.equal(env.LECTURN_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.LECTURN_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.LECTURN_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { LECTURN_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          lecturnHome: undefined,
          browser: undefined,
        ...
        assert.equal(env.LECTURN_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (LECTURN_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/LECTURN_HOST has to be probed as well.
    ...
    // Only the backend honours --host/LECTURN_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { LECTURN_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["LECTURN_PORT_OFFSET", "LECTURN_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .lecturn > ambient LECTURN_HOME)
    ...
    it.effect("ignores an ambient LECTURN_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { LECTURN_HOME: "/home/user/.lecturn" },
          serverOffset: 0,
          webOffset: 0,
          lecturnHome: undefined,
          browser: undefined,
    ...
    // defaults: an explicit LECTURN_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { LECTURN_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults LECTURN_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.LECTURN_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit LECTURN_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.LECTURN_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves LECTURN_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.LECTURN_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("lecturn home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "lecturn-devrunner-"));
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, lecturnHome: input.lecturnHome }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            Effect.provideService(HostProcessWorkingDirectory, input.cwd),
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { LECTURN_HOME: input.ambientHome },
            ),
          );

          return captured?.LECTURN_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            lecturnHome: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.lecturn",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".lecturn"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .lecturn over an ambient LECTURN_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient LECTURN_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            lecturnHome: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.lecturn"));
        }),
      );

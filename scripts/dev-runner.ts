...
import { resolveGitWorktreePath, resolveWorktreeLecturnHome } from "@lecturn/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@lecturn/shared/hostProcess";
...
export const DEFAULT_LECTURN_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".lecturn"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@lecturn/contracts",
    "--filter=@lecturn/web",
    "--filter=lecturn",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=lecturn", "dev"],
  "dev:web": ["run", "--filter=@lecturn/web", "dev"],
  "dev:desktop": ["run", "--filter=@lecturn/desktop", "--filter=@lecturn/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`LECTURN_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("LECTURN_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("LECTURN_PORT_OFFSET"),
  devInstance: optionalStringConfig("LECTURN_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `LECTURN_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric LECTURN_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed LECTURN_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_LECTURN_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly lecturnHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .lecturn > ambient LECTURN_HOME) is resolved
    // by the caller; an unset lecturnHome here genuinely means "use the default".
    const configuredBaseDir = lecturnHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.LECTURN_HOME = resolvedBaseDir;
    } else {
      delete output.LECTURN_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed lecturn service (an
    // agent working inside Lecturn), these leak through and the child server
    // fails startup with "The service launcher started a different lecturn version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.LECTURN_SERVICE_LAUNCHER_CONTEXT;
    delete output.LECTURN_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.LECTURN_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.LECTURN_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.LECTURN_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.LECTURN_PORT = String(serverPort);
      ...
      delete output.LECTURN_SINGLE_ORIGIN_DEV;
      delete output.LECTURN_MODE;
      delete output.LECTURN_NO_BROWSER;
      delete output.LECTURN_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.LECTURN_HOST = host;
    }

    if (!isDesktopMode) {
      output.LECTURN_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.LECTURN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.LECTURN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.LECTURN_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.LECTURN_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.LECTURN_MODE = "web";
      delete output.LECTURN_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`LECTURN_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["LECTURN_PORT_OFFSET", "LECTURN_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.lecturn` — see @lecturn/shared/devHome for why this must
    // outrank an ambient LECTURN_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeLecturnHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedLecturnHome =
      (input.lecturnHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.LECTURN_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      lecturnHome: resolvedLecturnHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.LECTURN_HOME ?? (yield* DEFAULT_LECTURN_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.LECTURN_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.LECTURN_DEV_ALLOWED_ORIGINS = [
            env.LECTURN_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so LECTURN_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.LECTURN_BUNDLED_DEV === undefined) {
            env.LECTURN_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  lecturnHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Lecturn data directory; runtime state is stored under userdata (equivalent to LECTURN_HOME). Inside a git worktree this defaults to that worktree's own .lecturn so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to LECTURN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("LECTURN_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to LECTURN_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("LECTURN_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to LECTURN_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("LECTURN_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to LECTURN_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("LECTURN_PORT")),
  ),

...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Lecturn");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Lecturn (Nightly)");
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.com.cloudgatherer.lecturn</string>");
  ...
  it("requires the Linux Stave member in the WSL archive only when it was staged", () => {
    ...
    assert.deepStrictEqual(
      ...
      [
        ...
        "node_modules/node-pty/prebuilds/linux-arm64/t3code-wsl-node-pty.json",
      ],
    );
    assert.deepStrictEqual(

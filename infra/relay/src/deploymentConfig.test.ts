...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("lecturn-relay-traces", "prod")).toBe(
      "lecturn-relay-traces-prod",
    );
    expect(relayResourceNameForStage("lecturn-relay-traces", "dev_julius")).toBe(
      "lecturn-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "lecturnrelay.com", hash)).toBe(
      "prod-abcdef0123456789.lecturnrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "lecturnrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });

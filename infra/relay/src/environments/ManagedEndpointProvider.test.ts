...
const config = RelayConfiguration.RelayConfiguration.of({
  ...
  managedEndpointBaseDomain: "lecturn.test",
...
function expectedManagedHostname(environmentId: string, userId = "user_ABC"): string {
  const hash = NodeCrypto.createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 16);
  return `dev-julius-${hash}.lecturn.test`;
}
...
function expectedManagedTunnelName(environmentId: string, userId = "user_ABC"): string {
  const hash = NodeCrypto.createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 16);
  return `lecturnrelay-managedendpoint-dev-julius-${hash}`;
}
...
describe("ManagedEndpointProvider", () => {
  ...
  it.effect("uses stage-scoped stable names without leaking unusual environment ids", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(requestedName).toMatch(/^lecturnrelay-managedendpoint-dev-julius-[a-f0-9]{16}$/);
      const configBody = (
      ...
      expect(configBody).toMatchObject({
        ingress: [
          {
            hostname: expect.stringMatching(/^dev-julius-[a-f0-9]{16}\.lecturn\.test$/),
          },
          { service: "http_status:404" },
        ],
      });
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const hash = NodeCrypto.createHash("sha256")
          ...
          .digest("hex")
          .slice(0, 16);
        const publicHostname = `${hash}-g-dev-julius.lecturn.test`;
        const originHostname = `gw-origin-dev-julius-${hash}.lecturn.test`;

...
export const makeTeamStore = Effect.gen(function* () {
  ...
  const get = (organizationId: string) =>
    query(
      sql<TeamAccount>`SELECT * FROM relay_team_accounts WHERE organization_id=${organizationId}`,
  ...
  const lock = Effect.fn("TeamStore.lock")(function* (
    ...
    const row = (yield* query(
      sql<TeamAccount>`SELECT * FROM relay_team_accounts WHERE organization_id=${organizationId} FOR UPDATE`,
    ))[0];
    if (!row) return yield* new TeamError({ code: "not_found", message: "Organization not found" });
    if (!ignoreBillingLease && row.billing_lease_expires_at > (yield* now))
  ...
  const seats = (organizationId: string) =>
    query(
      sql<TeamSeat>`SELECT * FROM relay_team_seats WHERE organization_id=${organizationId} ORDER BY assigned_at,user_id`,
    );
  ...
  const funding = (userId: string, environmentId: string) =>
    query(
      sql<TeamFunding>`SELECT * FROM relay_team_environment_funding WHERE user_id=${userId} AND environment_id=${environmentId}`,
    ).pipe(Effect.map((rows) => rows[0] ?? null));
  return {
    get,
    seats,
    ...
    history: (organizationId: string) =>
      query(
        sql<TeamAudit>`SELECT * FROM relay_team_audit WHERE organization_id=${organizationId} ORDER BY created_at DESC,id DESC LIMIT 100`,
      ),
  };
});

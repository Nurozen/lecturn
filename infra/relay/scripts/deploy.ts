...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    LECTURN_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    LECTURN_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    LECTURN_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    LECTURN_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    LECTURN_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    LECTURN_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("LECTURN_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("LECTURN_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Lecturn relay through Alchemy."));

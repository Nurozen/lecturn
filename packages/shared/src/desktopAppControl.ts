 ...
 * The state directory is hashed so custom Lecturn homes cannot exceed Unix socket
 * path limits.
 */
export function resolveDesktopAppControlAddress(input: {
  ...
  if (input.platform === "win32") {
    return {
      address: `\\\\.\\pipe\\lecturn-app-${stateHash}`,
      directory: null,
    };
  }
  ...
  const directory = input.joinPath(input.tempDir, `lecturn-${userKey}`);
  return {

...
describe("connectAuth", () => {
  it("round-trips state and challenge through the authorize URL fragment", () => {
    const url = buildConnectAuthorizeRequestUrl({
      hostedAppUrl: "https://lecturn.cloudgatherer.net",
      state: "q7mK9xV2pL4nR8sT6wYzAQ",
      challenge: "E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM",
    });
    const parsed = new URL(url);

    expect(parsed.origin).toBe("https://lecturn.cloudgatherer.net");
  ...
  it("rejects authorize requests missing state or challenge", () => {
    expect(
      readConnectAuthorizeRequest(new URL("https://lecturn.cloudgatherer.net/connect")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(new URL("https://lecturn.cloudgatherer.net/connect#state=abc")),
    ).toBeNull();
    expect(
      readConnectAuthorizeRequest(
        new URL("https://lecturn.cloudgatherer.net/connect#challenge=abc"),
      ),
    ).toBeNull();
  });
  ...
  it("rejects authorize requests whose loopback port is corrupted", () => {
    for (const port of ["", "abc", "-1", "0", "65536", "34338x", "34 38"]) {
      const url = new URL(
        `https://lecturn.cloudgatherer.net/connect#state=state-1&challenge=challenge-1&port=${encodeURIComponent(port)}`,
      );
  ...
  it("builds a PKCE authorize URL against the Clerk endpoint", () => {
    ...
    expect(url.searchParams.get("redirect_uri")).toBe(
      "https://lecturn.cloudgatherer.net/connect/callback",
    );

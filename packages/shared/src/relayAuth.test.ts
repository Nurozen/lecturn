...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(
      clerkFrontendApiHostnameFromPublishableKey(
        clerkPublishableKey("clerk.lecturn.cloudgatherer.net"),
      ),
    ).toBe("clerk.lecturn.cloudgatherer.net");
    expect(
      clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.lecturn.cloudgatherer.net")),
    ).toBe("https://clerk.lecturn.cloudgatherer.net");
  });
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.lecturn.cloudgatherer.net/path";

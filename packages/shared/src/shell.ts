...
const PATH_CAPTURE_START = "__LECTURN_PATH_START__";
const PATH_CAPTURE_END = "__LECTURN_PATH_END__";
...
export const SpawnExecutableResolution = Context.Reference<SpawnExecutableResolver>(
  "@lecturn/shared/shell/SpawnExecutableResolution",
  {
...
function envCaptureStart(name: string): string {
  return `__LECTURN_ENV_${name}_START__`;
}

function envCaptureEnd(name: string): string {
  return `__LECTURN_ENV_${name}_END__`;
}
...
export const readEnvironmentFromLoginShell: ShellEnvironmentReader = (
  ...
  const output = execFile(shell, ["-ilc", buildEnvironmentCaptureCommand(names)], {
    encoding: "utf8",
    timeout: 5000,
    // Interactive shell startup may ignore SIGTERM. This disposable probe must
    // exit at its deadline so environment discovery cannot block app startup.
    killSignal: "SIGKILL",
  });
...
export const WindowsShellEnvironment = Context.Reference<WindowsShellEnvironmentReader>(
  "@lecturn/shared/shell/WindowsShellEnvironment",
  {
...
export const CommandAvailability = Context.Reference<CommandAvailabilityChecker>(
  "@lecturn/shared/shell/CommandAvailability",
  {
...
export const CommandResolutionCache = Context.Reference<Map<string, CommandResolutionCacheEntry>>(
  "@lecturn/shared/shell/CommandResolutionCache",
  {

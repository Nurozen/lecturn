...
import {
  ...
  resolveRemoteLecturnCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote lecturn package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteLecturnCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "lecturn@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteLecturnCliPackageSpec({

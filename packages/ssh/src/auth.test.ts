...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "lecturn-ssh-askpass-test-" });
      ...
      assert.equal(env.LECTURN_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "lecturn");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\lecturn-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\lecturn-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(

...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@lecturn/contracts";
...
const PUBLISHABLE_LECTURN_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteLecturnCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_LECTURN_VERSION_PATTERN.test(appVersion)) {
    return `lecturn@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "lecturn@nightly";
  }

  return input.updateChannel === "nightly" ? "lecturn@nightly" : "lecturn@latest";
}

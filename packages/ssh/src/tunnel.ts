...
} from "@lecturn/shared/httpReadiness";
import * as NetService from "@lecturn/shared/Net";
import { extractJsonObject, fromLenientJson } from "@lecturn/shared/schemaJson";
import { satisfiesSemverRange } from "@lecturn/shared/semver";
...
export interface RemoteLecturnRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteLecturnRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteLecturnRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  LECTURN_NODE_ENGINE_RANGE=@@LECTURN_NODE_ENGINE_RANGE@@
  if [ -z "$LECTURN_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$LECTURN_NODE_ENGINE_RANGE" <<'NODE'
@@LECTURN_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for LECTURN_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$LECTURN_NODE_BIN/node" ]; then
        PATH="$LECTURN_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@LECTURN_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
LECTURN_NODE_SCRIPT_PATH=@@LECTURN_NODE_SCRIPT_PATH@@
if [ -n "$LECTURN_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$LECTURN_NODE_SCRIPT_PATH" "$@"
fi
if command -v lecturn >/dev/null 2>&1; then
  exec lecturn "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (lecturn depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a lecturn executable. \`npx --yes\` then exits 0 without
...
require_installed_lecturn_cli() {
  LECTURN_CLI_PATH="$("$@" -- sh -c 'command -v lecturn' || true)"
  if [ -n "$LECTURN_CLI_PATH" ]; then
    return 0
  fi
...
@@LECTURN_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.lecturn/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.lecturn"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-example.test"
RUNNER_NEXT="$STATE_DIR/run-lecturn.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@LECTURN_DEFAULT_REMOTE_PORT@@" "@@LECTURN_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@LECTURN_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@LECTURN_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@LECTURN_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@LECTURN_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@LECTURN_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@LECTURN_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env LECTURN_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@LECTURN_READY_TIMEOUT_MS@@"; then
    printf 'Remote Lecturn server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.lecturn/ssh-launch/@@LECTURN_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.lecturn"
RUNNER_FILE="$STATE_DIR/run-example.test"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteLecturnRunnerScript(input?: RemoteLecturnRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "lecturn@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      LECTURN_PACKAGE_SPEC: packageSpec,
      LECTURN_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      LECTURN_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemoteLecturnRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      LECTURN_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      LECTURN_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemoteLecturnRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    LECTURN_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    LECTURN_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteLecturnRunnerScript(input)),
    LECTURN_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    LECTURN_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    LECTURN_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    LECTURN_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    LECTURN_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    LECTURN_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    LECTURN_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteLecturnRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    LECTURN_STATE_KEY: remoteStateKey(target),
    LECTURN_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteLecturnRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteLecturnRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteLecturnRunnerOptions;
...
>()("@lecturn/ssh/tunnel/SshEnvironmentManager") {

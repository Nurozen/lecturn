...
import { classifyMarkdownImageSource } from "@lecturn/client-runtime/markdown-images";
import { resolveMediaSource } from "@lecturn/client-runtime/media-source";
...
const LECTURN_MCP_TOOL_LABELS: Record<
  string,
...
> = {
  ...
  create_threads: ["Create", "Creating", "Created", "Lecturn threads"],
  lecturn_thread_start: ["Start", "Starting", "Started", "a Lecturn thread"],
  lecturn_thread_list: ["List", "Listing", "Listed", "Lecturn threads"],
  lecturn_thread_read: ["Read", "Reading", "Read", "a Lecturn thread"],
  lecturn_thread_send: ["Send", "Sending", "Sent", "to a Lecturn thread"],
  lecturn_thread_wait: ["Wait", "Waiting", "Waited", "for a Lecturn thread"],
  lecturn_thread_interrupt: ["Interrupt", "Interrupting", "Interrupted", "a Lecturn thread"],
  lecturn_worktree_handoff: ["Hand off", "Handing off", "Handed off", "thread to a git worktree"],
  lecturn_worktree_status: ["Get", "Getting", "Got", "thread worktree status"],
...
function resolveLecturnMcpToolPresentation(value: string | undefined, status: string | undefined) {
  if (!value) return null;
  const name = normalizeCompactToolLabel(value).replace(
    /^(?:mcp__lecturn(?:_code)?__|lecturn(?:[-_]code)?(?:[.:/]|\s*·\s*))/i,
    "",
  );
  if (!Object.hasOwn(LECTURN_MCP_TOOL_LABELS, name)) return null;

  const [action, running, completed, detail] = LECTURN_MCP_TOOL_LABELS[name]!;
  const verb =
    status === "inProgress"
      ? running
      : status === "completed"
  ...
  return {
    ...
    icon: name.startsWith("preview_") ? ("browser" as const) : ("lecturn" as const),
  };
}
...
export function resolveWorkEntryToolPresentation(
  ...
  if (data !== null && typeof data === "object") {
    if (
      ...
      return resolveLecturnMcpToolPresentation(`${data.server}.${data.tool}`, status);
    }
    if ("toolName" in data && typeof data.toolName === "string") {
      return resolveLecturnMcpToolPresentation(data.toolName, status);
    }
  }

  return (
    resolveLecturnMcpToolPresentation(entry.toolTitle, status) ??
    resolveLecturnMcpToolPresentation(entry.label, status)
  );
}

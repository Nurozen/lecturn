...
describe("resolveMediaSource", () => {
  describe("direct URLs", () => {
    ...
    it.each(["data:image/png;base64,AAAA", "blob:https://lecturn.cloudgatherer.net/id"])(
      "loads %s directly",
      (href) => {
        expect(resolveMediaSource(href, { threadId, imageEmbed: true })).toMatchObject({
          access: "direct",
  ...
  it("serves Lecturn attachment files in place like any other host path", () => {
    const path = `/home/demo/.lecturn/userdata/attachments/${attachmentId}.mp4`;

...
}).annotate({ description: "Links an authenticated cloud user to a Lecturn environment." });
...
>()("@lecturn/contracts/relay/RelayClientPrincipal") {}
...
>()("@lecturn/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Lecturn Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["lecturn-mobile", "lecturn-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "lecturn-mobile" as const;
export const RelayWebClientId = "lecturn-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Lecturn environments, connecting authorized clients, and publishing agent activity.",
  );

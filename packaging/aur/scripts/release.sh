...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Lecturn-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit lecturn-icon.png
git config user.name 'lecturn-ci'
git config user.email 'lecturn-ci@users.noreply.github.com'
git add -A

...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Lecturn once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.lecturn database. Use an isolated --base-dir.";
  }
}
...
export const lecturnSqliteStateCommand = Command.make(
  "lecturn-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Lecturn base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Lecturn SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(lecturnSqliteStateCommand, { version: "0.0.0" }).pipe(

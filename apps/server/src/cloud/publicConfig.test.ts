...
it.effect("prefers a runtime relay URL override over the statically injected value", () =>
  Effect.gen(function* () {
    const relayUrl = yield* makeRelayUrlConfig("https://embedded.example.test").pipe(
      provideEnv({ LECTURN_RELAY_URL: "https://runtime.example.test///" }),
    );
...
it.effect("rejects an insecure runtime relay URL override", () =>
  makeRelayUrlConfig("https://embedded.example.test").pipe(
    provideEnv({ LECTURN_RELAY_URL: "http://runtime.example.test" }),
    Effect.flip,
  ),
);
  ...
  Effect.gen(function* () {
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ LECTURN_HOSTED_APP_URL: "https://nightly.lecturn.cloudgatherer.net" }),
      ),
      "https://nightly.lecturn.cloudgatherer.net",
    );
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ LECTURN_HOSTED_APP_URL: "http://localhost:5733" }),
      ),
      "http://localhost:5733",
    );
  }),
...
it.effect("rejects malformed or insecure hosted app URLs", () =>
  Effect.gen(function* () {
    for (const value of [
      "lecturn.cloudgatherer.net",
      "http://lecturn.cloudgatherer.net",
      "https://lecturn.cloudgatherer.net/nested",
      "https://lecturn.cloudgatherer.net?alias=true",
    ]) {
      const result = yield* hostedAppUrlConfig.pipe(
        provideEnv({ LECTURN_HOSTED_APP_URL: value }),
        Effect.result,
      );
...
it.effect("prefers runtime Clerk OAuth config overrides over statically injected values", () =>
  Effect.gen(function* () {
    const config = yield* makeCloudCliOAuthConfig({
      ...
      provideEnv({
        LECTURN_CLERK_PUBLISHABLE_KEY: "pk_test_cnVudGltZS5leGFtcGxlLnRlc3Qk",
        LECTURN_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_runtime",
      }),
    );
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
  ...
  assert.deepEqual(
    resolveRelayClientTracingConfig(
      {
        LECTURN_RELAY_CLIENT_OTLP_TRACES_URL: "https://runtime.example.test/v1/traces",
        LECTURN_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        LECTURN_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ),
    {
  ...
  assert.equal(
    resolveRelayClientTracingConfig(
      {
        LECTURN_RELAY_CLIENT_OTLP_TRACES_URL: "http://insecure.example.test/v1/traces",
        LECTURN_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        LECTURN_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ...
    Effect.gen(function* () {
      ...
      assert.equal(
        yield* makeHostedAppUrlConfig("https://lecturn.example.test").pipe(
          provideEnv({ LECTURN_HOSTED_APP_URL: "https://override.example.test" }),
        ),

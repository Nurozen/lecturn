  ...
  it.effect("does not accept an unauthenticated request trace parent", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("https://environment.example.test/api/lecturn-cloud/mint-credential", {
          headers: {
...
describe("releaseManagedTunnelOnShutdown", () => {
  ...
  it.effect("still releases a pending update when the launcher is stopping", () => {
    // `lecturn service uninstall` or `systemctl stop` during the pending window:
...
describe("link proof provider kinds", () => {
  ...
  it("accepts managed and manual endpoints but not lecturn_relay", () => {
    ...
    expect(isSupportedLinkProviderKind(proofRequest("lecturn_relay"))).toBe(false);
  });

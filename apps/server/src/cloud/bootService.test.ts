  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable lecturn.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart lecturn.service") control.active = true;
      if (
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(commands).not.toContain("systemctl --user stop lecturn.service");
        }),
    );
      ...
      Effect.gen(function* () {
        ...
        for (const allowDowngrade of [false, true]) {
          ...
          expect(
            commands.filter(
              (command) =>
                command.startsWith("systemctl ") && !command.includes("show-environment"),
            ),
          ).toEqual([
            "systemctl --user stop lecturn.service",
            "systemctl --user restart lecturn.service",

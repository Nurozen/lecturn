...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("reuses the cached Git root for repeated workspace lookups", () => {
        ...
        Effect.sync(() => {
          ...
          return {
            stdout: input.args.includes("rev-parse")
              ? "/repo\n"
              : "origin\tgit@github.com:Nurozen/lecturn.git (fetch)\n",
            stderr: "",
    ...
    return Effect.gen(function* () {
      ...
      expect(first?.canonicalKey).toBe("github.com/nurozen/lecturn");
  ...
  it.effect("returns null without spawning git when the workspace root is a Stave space", () => {
    const calls: Array<ReadonlyArray<string>> = [];
    const processRunner = Layer.succeed(ProcessRunner.ProcessRunner, {
      run: (input) =>
        Effect.sync(() => {
    ...
    return Effect.gen(function* () {
      ...
      const spaceRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "lecturn-repository-identity-stave-root-",
      });
      yield* fileSystem.writeFileString(
        path.join(spaceRoot, ".stave.yaml"),
        "version: 1\nid: space\nrepos: []\n",
      );
      const checkout = path.join(spaceRoot, "t3code");
      yield* fileSystem.makeDirectory(checkout);

      const resolver = yield* RepositoryIdentityResolver.RepositoryIdentityResolver;
      expect(yield* resolver.resolve(spaceRoot)).toBeNull();
      expect(calls).toEqual([]);

      // Checkouts inside the space carry no manifest and resolve as usual.
      const nested = yield* resolver.resolve(checkout);
      expect(nested?.canonicalKey).toBe("github.com/t3tools/t3code");
      expect(calls).toEqual([
        ["-C", checkout, "rev-parse", "--show-toplevel"],
        ["-C", "/space/t3code", "remote", "-v"],
      ]);
    }).pipe(Effect.provide(resolverLayer));
  });
  ...
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "lecturn-repository-identity-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:Nurozen/lecturn.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/nurozen/lecturn");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("nurozen/lecturn");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("nurozen");
      expect(identity?.name).toBe("lecturn");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      const repoRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "lecturn-repository-identity-nested-root-test-",
      });
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:Nurozen/lecturn.git"]);
  ...
  it.effect("returns null for non-git folders and repos without remotes", () =>
    Effect.gen(function* () {
      ...
      const nonGitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "lecturn-repository-identity-non-git-",
      });
      const gitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "lecturn-repository-identity-no-remote-",
      });
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "lecturn-repository-identity-upstream-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/lecturn.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:Nurozen/lecturn.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "lecturn-repository-identity-nested-group-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:Nurozen/platform/lecturn.git"]);

      const resolver = yield* RepositoryIdentityResolver.RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/nurozen/platform/lecturn");
      expect(identity?.displayName).toBe("nurozen/platform/lecturn");
      expect(identity?.owner).toBe("nurozen");
      expect(identity?.name).toBe("lecturn");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "lecturn-repository-identity-late-remote-test-",
        });
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/nurozen/lecturn");
        expect(refreshedIdentity?.name).toBe("lecturn");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "lecturn-repository-identity-remote-change-test-",
      });
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/nurozen/lecturn");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:Nurozen/lecturn-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/nurozen/lecturn");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/nurozen/lecturn-next");
      expect(refreshedIdentity?.displayName).toBe("nurozen/lecturn-next");
      expect(refreshedIdentity?.name).toBe("lecturn-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),

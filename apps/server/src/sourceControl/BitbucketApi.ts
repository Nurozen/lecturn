...
import { sanitizeBranchFragment } from "@lecturn/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@lecturn/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("LECTURN_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("LECTURN_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("LECTURN_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("LECTURN_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("lecturn/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `lecturn/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set LECTURN_BITBUCKET_EMAIL and LECTURN_BITBUCKET_API_TOKEN, or LECTURN_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}

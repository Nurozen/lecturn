...
export interface TelemetryCounters {
  ...
  readonly allLecturn: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allLecturn: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allLecturn = input.counters.allLecturn;
  for (const processDelta of input.deltas) {
    ...
    allLecturn = incrementCounters(allLecturn, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allLecturn = incrementCounters(allLecturn, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allLecturn = incrementCounters(allLecturn, { processExits: 1 });
  }

  return { backend, electron, monitor, allLecturn };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allLecturn: aggregate(ordered, counters.allLecturn),
    },
    deltas: processDeltas,
  };
}

/**
 * All text `lecturn triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Lecturn triage playbook

You are a support engineer for Lecturn (https://github.com/Nurozen/lecturn), working
...
Fetch https://raw.githubusercontent.com/Nurozen/lecturn/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/Nurozen/lecturn <source-cache-dir>/<hash>
...
- How is Lecturn running on this machine: \`lecturn serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (lecturn.cloudgatherer.net), the
...
Search existing issues in Nurozen/lecturn (use \`gh\`, or the public GitHub search
...
Do not patch the Lecturn source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/Nurozen/lecturn/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Lecturn triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Lecturn user is \
having a problem with their install and started this session with \`lecturn triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Lecturn triage context

Generated by \`lecturn triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/Nurozen/lecturn

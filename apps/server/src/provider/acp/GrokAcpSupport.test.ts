...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no Lecturn runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when Lecturn is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the Lecturn referrer through Grok OAuth env", () => {

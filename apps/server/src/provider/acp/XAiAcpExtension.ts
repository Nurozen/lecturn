...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@lecturn/contracts";
...
/**
 * Client captured the plan for Lecturn's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via Lecturn UI.
 */
 ...
 * Used so Lecturn can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `lecturn-xai-prompt-${nextPromptFallbackId}`;
    });

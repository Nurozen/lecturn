 ...
 * Enable with: LECTURN_GROK_ACP_PROBE=1 vp test run GrokAcpCliProbe
 * Set LECTURN_GROK_LIVE_TURN=1 to also send a small prompt to the real model.
 *
...
const makeProbeRuntime = Effect.gen(function* () {
  ...
  return yield* makeGrokAcpRuntime({
    ...
    childProcessSpawner,
    cwd: process.cwd(),
    clientInfo: { name: "lecturn-grok-probe", version: "0.0.0" },
  });
});

describe.runIf(process.env.LECTURN_GROK_ACP_PROBE === "1")("Grok ACP CLI probe", () => {
  ...
  it.effect.skipIf(process.env.LECTURN_GROK_LIVE_TURN !== "1")(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const result = yield* runtime.prompt({
          prompt: [{ type: "text", text: "Reply exactly GROK_LECTURN_OK. Do not use any tools." }],
        });
        yield* runtime.drainEvents;
        expect(result.stopReason).toBe("end_turn");
        expect(chunks.join("")).toContain("GROK_LECTURN_OK");

...
const makeHarness = Effect.fn("makeAntigravityDriverHarness")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "lecturn-antigravity-driver-" });
  ...
  const controls = {
    selected: first,
    failResolution: options.missingInstallation ?? false,
  ...
  const instance = yield* AntigravityDriver.create({
    ...
    environment: [
      ...
      { name: "LECTURN_ACP_ANTIGRAVITY", value: "1" },
      { name: "LECTURN_ACP_REQUEST_LOG_PATH", value: requestLog },
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // Runtime discoveries remain visible even when the manifest predates them.

...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    // Resolve the nearest existing ancestor for new nested files, including
    // aliases such as macOS /var. Existing leaf symlinks must resolve too.
      ...
      const canonical = yield* input.fileSystem.realPath(ancestor).pipe(
        ...
        Effect.catch((error) =>
          error.reason._tag === "NotFound"
            ...
            : Effect.fail(
                EffectAcpErrors.AcpRequestError.invalidParams("Cannot resolve file path."),
              ),
        ),
      );
      if (Option.isSome(canonical)) {
        real = path.join(canonical.value, path.relative(ancestor, resolved));
        break;
      }
      const link = yield* input.fileSystem.readLink(ancestor).pipe(Effect.option);
      const parent = path.dirname(ancestor);
      if (Option.isSome(link) || parent === ancestor) {
        return yield* EffectAcpErrors.AcpRequestError.invalidParams("Cannot resolve file path.");
      }
      ancestor = parent;
    }

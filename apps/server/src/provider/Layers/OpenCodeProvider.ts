        ...
        probe: {
          ...
          message:
            openCodeSettings.serverUrl.trim().length > 0
              ? "OpenCode is disabled in Lecturn settings. A server URL is configured."
              : "OpenCode is disabled in Lecturn settings.",
        },
      });
    }
...
> {
  ...
  if (!isExternalServer) {
    ...
    if (!version) {
      return fallback(
        new Error(
          `Unable to determine OpenCode version from \`opencode --version\` output. Lecturn requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
        ),
        null,
      );
    }

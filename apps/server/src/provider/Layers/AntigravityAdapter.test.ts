...
const layer = ServerConfig.layerTest(process.cwd(), {
  prefix: "lecturn-antigravity-adapter-test-",
...
it.layer(layer)("AntigravityAdapter", (it) => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "lecturn-antigravity-transport-",
        });
          ...
          makeRuntime: (input) =>
            makeAntigravityAcpRuntime({
              ...
              spawn: {
                ...
                env: {
                  ...process.env,
                  LECTURN_ACP_ANTIGRAVITY: "1",
                  LECTURN_ACP_REQUEST_LOG_PATH: requestLog,
                },
                extendEnv: false,
              },
  ...
  it.effect("rejects a fork before launching a native session", () =>
    Effect.gen(function* () {
      ...
      const result = yield* h.adapter
        .startSession({
          ...
          fork: {
            sourceResumeCursor: { sessionId: "parent" },
            sourceProviderInstanceId: instanceId,
            providerTurnRef: null,
            throughTurnId: TurnId.make("parent-turn"),
  ...
  it.effect("serves client file reads and writes only inside the session roots", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fs.makeTempDirectoryScoped({ prefix: "lecturn-agy-fs-" });
      const outside = yield* fs.makeTempDirectoryScoped({ prefix: "lecturn-agy-outside-" });
      ...
      yield* fs.symlink(path.join(outside, "secret.txt"), path.join(cwd, "leaf-link"));
      ...
      for (const requestPath of [
        path.join(cwd, "leaf-link"),
        path.join(cwd, "directory-link", "nested", "new.txt"),
        path.join(cwd, "dangling-link"),
      ]) {
        expect(
          (yield* write({
            sessionId: nativeSessionId,
      ...
      expect(yield* fs.exists(path.join(outside, "nested", "new.txt"))).toBe(false);

...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir =
      input?.baseDir ?? NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "lecturn-reactor-"));
  ...
  it("does not auto-title a forked thread on a /compact turn but titles the next real turn", async () => {
    const harness = await createHarness();
    harness.generateThreadTitle.mockReturnValue(
      Effect.succeed({ title: "Fix reconnect handling" }),
    );
    ...
    await dispatchThreadFork(harness);
    // The first new turn after the fork is a compaction command. It must not
    // count as the child's first conversation turn.
    await harness.runEffect(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-fork-compact"),
        threadId: forkChildThreadId,
        message: {
          messageId: asMessageId("user-message-fork-compact"),
          role: "user",
          text: "/compact",
          attachments: [],
        },
    ...
    await harness.drain();
    expect(harness.compactThread.mock.calls[0]?.[0]).toBe(forkChildThreadId);
    expect(harness.sendTurn).not.toHaveBeenCalled();
    expect(harness.generateThreadTitle).not.toHaveBeenCalled();

    // The following real message is the child's first post-fork turn.
    await harness.runEffect(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-turn-start-fork-after-compact"),
        threadId: forkChildThreadId,
        message: {
          messageId: asMessageId("user-message-fork-after-compact"),
          role: "user",
          text: "Continue from the fork point.",
          attachments: [],
        },
  ...
  it("pins the first user message when regeneration context is truncated", async () => {
    ...
    expect(message).not.toContain("lecturn-citation://");

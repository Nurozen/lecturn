# Lecturn Mobile Terminal Native Module
...
custom-I/O and live-padding branch. `LecturnTerminalView` owns a `libghostty` surface and uses that
callback I/O model:
...
https://github.com/Yash-Singh1/ghostty checked out at that revision, based on VVTerm's `vvterm/custom-io-padding` branch.

```bash
apps/mobile/modules/lecturn-terminal/scripts/build-libghostty-ios16.sh
```
...
```bash
apps/mobile/modules/lecturn-terminal/scripts/build-libghostty-android.sh
```

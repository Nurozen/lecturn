...
public final class LecturnKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("LecturnKeyboardCommands")

    View(LecturnKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: LecturnKeyboardCommandsView, commands: [String]) in
...
public final class LecturnKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.lecturnFirstResponder == nil else { return }
...
private extension UIView {
  var lecturnFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.lecturnFirstResponder { return responder }
    }
    return nil
  }
}

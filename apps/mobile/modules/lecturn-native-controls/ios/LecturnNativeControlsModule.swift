...
public final class LecturnNativeControlsModule: Module {
  private let presentationSources = LecturnPresentationSources()
  private var videoPresentation: LecturnNativeVideoPresentation?
  private var filePresentation: LecturnNativeFilePresentation?

  public func definition() -> ModuleDefinition {
    Name("LecturnNativeControls")
    ...
    View(LecturnPresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: LecturnPresentationSourceView, identifier: String) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/LecturnShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("LecturnNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/LecturnShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "LecturnNativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = LecturnNativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func shareFile(url: URL, title: String, sourceIdentifier: String, promise: Promise) throws {
    guard let presenter = appContext?.utilities?.currentViewController() else {
      throw NSError(
        domain: "LecturnNativePresentation",
        code: 2,

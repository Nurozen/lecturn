...
final class LecturnNativeVideoPresentation: NSObject, AVPlayerViewControllerDelegate,
  ...
  func present(from presenter: UIViewController, sources: LecturnPresentationSources, sourceIdentifier: String) {
    let audioSession = AVAudioSession.sharedInstance()
    previousAudioSession = AudioSessionConfiguration(audioSession)
    do {
      try audioSession.setCategory(.playback, mode: .moviePlayback)
    } catch {
      NSLog("Lecturn video audio session: %@", error.localizedDescription)
    }

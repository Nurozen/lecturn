...
import { ProviderDriverKind } from "@lecturn/contracts";
...
describe("mobile slash commands", () => {
  ...
  it("still applies the Lecturn plan command for supported providers", () => {
    const items = buildComposerSlashCommandItems({
      query: "plan",
      atMessageStart: true,
      hasThread: true,
    ...
    const item = items[0];
    if (!item) throw new Error("Expected the Lecturn plan command");
    expect(
      resolveComposerCommandSelection({
        draftMessage: "/plan",

...
function ProviderSetupScreen({ environmentId, instanceId }: ProviderSetupRouteParams) {
  ...
  const usesCustomBinary = usesCustomAntigravityBinary(config?.settings, instanceId);
              ...
              <View className="gap-3 border-t border-border pt-4">
                {usesCustomBinary ? (
                  <Text className="text-sm text-foreground">
                    Clear this instance’s binary path in web or desktop provider settings to install
                    or remove the managed runtime.
                  </Text>
                ) : null}
                {installActive ? (
                  <>
                  ...
                  <SetupButton
                    ...
                    primary
                    disabled={controlsDisabled || authActive || usesCustomBinary}
                    onPress={() =>
                      void perform(
                        () => startInstall(target),
                    ...
                    {auth.authorizationUrl && auth.phase === "waiting" ? (
                      <>
                        ...
                        <TextInput
                          ...
                          autoComplete="off"
                          autoCorrect={false}
                          className="min-h-12 rounded-none border border-border bg-background px-3 py-3 text-base text-foreground"

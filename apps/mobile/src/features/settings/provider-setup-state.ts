...
/** Explicit instance configuration overrides legacy settings, even when its path is empty. */
export function usesCustomAntigravityBinary(
  settings: ServerSettings | undefined,
  instanceId: ProviderInstanceId,
): boolean {
  const config =
    settings?.providerInstances[instanceId]?.config ??
    (instanceId === "antigravity" ? settings?.providers.antigravity : undefined);
  return (
    config !== null &&
    typeof config === "object" &&
    "binaryPath" in config &&
    typeof config.binaryPath === "string" &&
    Boolean(config.binaryPath.trim())
  );
}

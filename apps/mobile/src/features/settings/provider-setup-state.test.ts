import { usesCustomAntigravityBinary } from "./provider-setup-state";
import {
...
describe("usesCustomAntigravityBinary", () => {
  it.each(["/missing/antigravity", "/custom/antigravity"])(
    "blocks managed changes regardless of whether %s exists",
    (binaryPath) => {
      const settings = {
        ...DEFAULT_SERVER_SETTINGS,
        providers: {
      ...
      expect(usesCustomAntigravityBinary(settings, ProviderInstanceId.make("antigravity"))).toBe(
        true,
      );
      expect(
        usesCustomAntigravityBinary(
          {
            ...settings,
            providers: {
              ...settings.providers,
              antigravity: { ...settings.providers.antigravity, binaryPath: "" },
            },
          },
  ...
  it("uses the selected explicit instance and allows an empty override of a legacy path", () => {
    const settings = {
      ...DEFAULT_SERVER_SETTINGS,
      providers: {
        ...DEFAULT_SERVER_SETTINGS.providers,
    ...
    expect(usesCustomAntigravityBinary(settings, ProviderInstanceId.make("work"))).toBe(true);
    expect(usesCustomAntigravityBinary(settings, ProviderInstanceId.make("another"))).toBe(false);
  });
});

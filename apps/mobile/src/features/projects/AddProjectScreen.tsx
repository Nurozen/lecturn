...
import {
  ...
  isWindowsPlatform,
} from "@lecturn/client-runtime/state/projects";
import {
  CommandId,
  type EnvironmentId,
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1 gap-0.5">
          <Text className="text-base leading-snug font-lecturn-bold">{props.title}</Text>
          {props.subtitle ? (
  ...
  return (
    ...
    >
      {props.loading ? (
        ...
        <Text className="text-base font-lecturn-bold text-primary-foreground">{props.label}</Text>
      )}
    </Pressable>
  );
}

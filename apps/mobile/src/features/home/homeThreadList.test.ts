...
describe("buildHomeThreadGroups", () => {
  ...
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        ...
        remoteName: "origin",
        remoteUrl: "git@github.com:nurozen/lecturn.git",
      },
    };
    const projects = [
      makeProject({
    ...
    expect(scopes[0]?.title).toBe("lecturn");
  ...
  it("routes stale duplicate project refs through the canonical repository group", () => {
    ...
    const local = makeProject({
      ...
      environmentId: localEnvironmentId,
      title: "lecturn",
      workspaceRoot: "/workspaces/lecturn",
      repositoryIdentity,
    });
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const projects = [
      makeProject({
        ...
        title: "Web",
        workspaceRoot: "/workspaces/lecturn/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/lecturn/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "Lecturn" })[0];

...
import { ProviderInstanceId, type ModelSelection, type ServerConfig } from "@lecturn/contracts";
...
describe("mobile model options", () => {
  ...
  describe("Antigravity selections", () => {
    ...
    const aliasSelection = { ...selection, model: "antigravity-default" };
    const aliasConfig = {
      ...config,
      providers: config.providers.map((provider) => ({
        ...provider,
        models: provider.models.map((model) => ({ ...model, aliases: [aliasSelection.model] })),
      })),
    };

    it("allows an advertised default alias and keeps its saved options and picker metadata", () => {
      const resolved = resolveDefaultableModelSelection(aliasConfig, aliasSelection);
      const options = buildModelOptions(aliasConfig, resolved);
      const chosen = resolveNewTaskModelSelection({
        draftSelection: null,
        projectDefaultSelection: resolved,
        stickySelection: null,
        modelOptions: options,
      });

      expect(chosen).toBe(aliasSelection);
      expect(isModelSelectionUnavailable(aliasConfig, chosen)).toBe(false);
      const fallback = options.find((option) => option.selection.model === aliasSelection.model);
      expect(fallback).toMatchObject({
        key: "google_work:antigravity-default",
        label: model.name,
        subtitle: "Google",
        capabilities: model.capabilities,
        isLegacy: true,
      });
      expect(fallback?.isUnavailable).not.toBe(true);
      expect(fallback?.selection).toBe(aliasSelection);
    });

    it("marks a withdrawn alias unavailable without replacing the saved selection", () => {
      expect(resolveDefaultableModelSelection(config, aliasSelection)).toBe(aliasSelection);
      expect(isModelSelectionUnavailable(config, aliasSelection)).toBe(true);
      const fallback = buildModelOptions(config, aliasSelection).find(
        (option) => option.selection.model === aliasSelection.model,
      );
      expect(fallback?.isUnavailable).toBe(true);
      expect(fallback?.selection).toBe(aliasSelection);
    });
    ...
    it.each([
      ...
      ["signed out", { auth: { status: "unauthenticated" } }],
      ["unavailable", { availability: "unavailable" }],
    ] as const)("does not let an advertised alias bypass a %s provider", (_state, update) => {
      const unavailableConfig = {
        ...aliasConfig,
        providers: aliasConfig.providers.map((provider) => ({ ...provider, ...update })),
      };
      expect(isModelSelectionUnavailable(unavailableConfig, aliasSelection)).toBe(true);
      const [fallback] = buildModelOptions(unavailableConfig, aliasSelection);
      expect(fallback?.isUnavailable).toBe(true);
      expect(fallback?.label).toBe(model.name);
      expect(fallback?.selection).toBe(aliasSelection);
    });

    it.each(["unknown-model", "ANTIGRAVITY-DEFAULT", " antigravity-default", model.name])(
      "does not treat unadvertised identifier %s as a supported alias",
      (identifier) => {
        const unknown = { ...aliasSelection, model: identifier };
        expect(isModelSelectionUnavailable(aliasConfig, unknown)).toBe(true);
        const fallback = buildModelOptions(aliasConfig, unknown).find(
          (option) => option.selection.model === identifier,
        );
        expect(fallback?.isUnavailable).toBe(true);
        expect(fallback?.capabilities).toBeNull();
        expect(fallback?.selection).toBe(unknown);
      },
    );

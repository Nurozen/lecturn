...
function findAdvertisedModel(
  models: LecturnServerConfig["providers"][number]["models"],
  selectedModel: string,
) {
  return (
    models.find((model) => model.slug === selectedModel) ??
    models.find((model) => model.aliases?.includes(selectedModel))
  );
}

/** Whether a known Antigravity selection needs setup or a different model. */
export function isModelSelectionUnavailable(
  config: LecturnServerConfig | null | undefined,
  ...
  return (
    ...
    (!provider ||
      ...
      !findAdvertisedModel(provider.models, selection.model))
  );
}
...
export function buildModelOptions(
  ...
  if (fallbackModelSelection) {
    ...
    if (existing) {
      ...
      const model = provider && findAdvertisedModel(provider.models, fallbackModelSelection.model);

...
vi.mock("@lecturn/client-runtime/state/runtime", () => ({
...
describe("prepareTurnAttachments", () => {
  ...
  it("uploads image bytes over HTTP while retaining the durable offline image", async () => {
    ...
    expect(mocks.writeFile).toHaveBeenCalledWith("file:///cache/lecturn-upload-uuid", "YWJj", {
      encoding: "base64",
    });
    expect(mocks.upload).toHaveBeenCalledWith(
      "file:///cache/lecturn-upload-uuid",
      "https://environment.example/api/attachments/upload/signed",
      expect.objectContaining({ headers: { "Content-Type": "image/png" } }),
    );
    expect(mocks.deleteFile).toHaveBeenCalledExactlyOnceWith("file:///cache/lecturn-upload-uuid");
    expect(prepared.status).toBe("ready");
    if (prepared.status !== "ready") return;
    expect(prepared.attachments).toEqual([
      {

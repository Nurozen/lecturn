...
import { BUILT_IN_THEME_IDS, type BuiltInThemeId } from "@lecturn/shared/themePalettes";
...
if (import.meta.main) {
  ...
  for (const [filename, contents] of getGeneratedUniwindThemeOutputs()) {
    if (checkOnly) {
      ...
      if (current !== contents) {
        console.error(
          `${NodePath.relative(process.cwd(), filename)} is stale. Run vp run --filter @lecturn/mobile generate.`,
        );
        process.exitCode = 1;
      }
      continue;

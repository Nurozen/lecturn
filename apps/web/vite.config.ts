...
const isSingleOriginDev = process.env.LECTURN_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_LECTURN_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.LECTURN_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: LECTURN_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); LECTURN_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.LECTURN_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // LECTURN_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.LECTURN_PORT, configuredWsUrl);
...
const configuredAllowedHosts = (process.env.LECTURN_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_LECTURN_RELAY_URL": JSON.stringify(configuredRelayUrl),

...
describe("hasCloudPublicConfig", () => {
  it("requires both public cloud values", () => {
    ...
    vi.stubEnv("VITE_LECTURN_RELAY_URL", "");
    ...
    vi.stubEnv("VITE_LECTURN_RELAY_URL", "https://relay.example.test");
  ...
  it("rejects an insecure relay URL", () => {
    vi.stubEnv("VITE_CLERK_PUBLISHABLE_KEY", "pk_test_example");
    vi.stubEnv("VITE_CLERK_JWT_TEMPLATE", "lecturn-relay");
    vi.stubEnv("VITE_LECTURN_RELAY_URL", "http://relay.example.test");
  ...
  it("reports the missing Clerk JWT template as structured configuration", () => {
    ...
    expect(() => resolveRelayClerkTokenOptions()).toThrowError(
      new CloudPublicConfigMissingError({ key: "LECTURN_CLERK_JWT_TEMPLATE" }),
    );
  });
});

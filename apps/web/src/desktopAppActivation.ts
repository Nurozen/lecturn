  ...
  if (projectId === null) {
    try {
      ...
      return failure(
        ...
        errorMessage(error, "Lecturn could not add the project."),
      );
    }
  }
  ...
  try {
    ...
    if (opened === null) {
      return failure(
        request.requestId,
        "thread-open-failed",
        "Lecturn could not open a new thread for the project.",
      );
    }
    return {
      version: 1,
    ...
    return failure(
      request.requestId,
      "thread-open-failed",
      errorMessage(error, "Lecturn could not open a new thread for the project."),
    );
  }
}

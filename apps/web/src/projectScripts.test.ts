...
import {
  ...
  setupProjectScript,
} from "@lecturn/shared/projectScripts";
...
describe("projectScripts helpers", () => {
  ...
  it("allows overriding runtime env values", () => {
    const env = projectScriptRuntimeEnv({
      project: { cwd: "/repo" },
      extraEnv: {
        LECTURN_PROJECT_ROOT: "/custom-root",
        CUSTOM_FLAG: "1",
      },
    });

    expect(env.LECTURN_PROJECT_ROOT).toBe("/custom-root");
    expect(env.CUSTOM_FLAG).toBe("1");
    expect(env.LECTURN_WORKTREE_PATH).toBeUndefined();
  });

import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@lecturn/contracts";
import type { ServerUpdateState } from "@lecturn/client-runtime/state/server";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "lecturn:version-mismatch-dismissals:v1";
...
/**
 * The skew a user can act on: the connected server runs an older Lecturn than
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Lecturn version.",
  };
}
...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx lecturn@${targetVersion}`;
}

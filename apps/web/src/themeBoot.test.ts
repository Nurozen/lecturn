...
const THEME_STORAGE_KEY = "lecturn:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Lecturn Chat follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "lecturn-chat", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Lecturn Chat",
      storage: {
        [THEME_STORAGE_KEY]: "lecturn-chat",
    ...
    {
      name: "a legacy lecturn-grove preference resolves through the alias",
      storage: { [THEME_STORAGE_KEY]: "lecturn-grove", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "legacy lecturn-chat-dark resolves to dark Lecturn Chat",
      storage: { [THEME_STORAGE_KEY]: "lecturn-chat-dark" },
      prefersDark: true,
    },
    {
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("lecturn-chat");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "lecturn:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("lecturn-chat");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(LECTURN_CHAT_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "lecturn-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "lecturn:theme-halves:v1": JSON.stringify({ dark: "lecturn-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "lecturn-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "lecturn:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("lecturn-chat");
    expect(boot.isDark).toBe(true);
  });

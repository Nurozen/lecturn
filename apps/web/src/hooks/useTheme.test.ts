...
describe("theme failure handling", () => {
  ...
  it("reads the persisted Lecturn Chat theme preference", async () => {
    vi.stubGlobal("window", {
      localStorage: createStorage({
        getItem: () => "lecturn-chat",
      }),
    });

    const { readThemePreference } = await import("./useTheme");

    expect(readThemePreference()).toBe("lecturn-chat");
  });
  ...
  it("falls back during initial theme application and logs only safe attributes", async () => {
    ...
    expect(errorLog).toHaveBeenCalledWith(
      "Failed to read theme preference for lecturn:theme.",
  ...
  it("retries a failed storage read only after a relevant storage event", async () => {
    ...
    const getItem = vi.fn((key: string) => (key === "lecturn:theme" ? themeGetItem() : null));
    const errorLog = vi.spyOn(console, "error").mockImplementation(() => {});
    ...
    storageHandler?.({ key: "lecturn:theme" } as StorageEvent);
    readSnapshot?.();

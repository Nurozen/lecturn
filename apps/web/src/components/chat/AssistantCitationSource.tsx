...
import type { AssistantCitation, MessageId, ScopedThreadRef } from "@lecturn/contracts";
...
const COMMENT_HIGHLIGHT_NAME = "lecturn-assistant-citation-comment";
...
export function observeAssistantCitationSource({
  ...
  const clear = () => {
    if (highlighted && CSS.highlights.get("lecturn-assistant-citation") === highlighted) {
      CSS.highlights.delete("lecturn-assistant-citation");
    }
    highlighted = null;
    ownedRange = null;
  ...
  const highlight = () => {
    ...
    if (!pulseAnimation) {
      ...
      pulseAnimation.id = "lecturn-assistant-citation-pulse";

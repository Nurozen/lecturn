...
export const ComposerPrimaryActions = memo(function ComposerPrimaryActions({
  ...
  const sendButton = (
    <button
      type="submit"
      className={cn(
        "lecturn-composer-send relative isolate flex h-9 w-9 items-center justify-center overflow-hidden rounded-full shadow-xs transition-all duration-150 enabled:cursor-pointer enabled:inset-shadow-[0_1px_--theme(--color-white/16%)] hover:scale-105 active:inset-shadow-[0_1px_--theme(--color-black/8%)] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none disabled:hover:scale-100 sm:h-8 sm:w-8",
    ...
    >
      ...
      {isConnecting || isSendBusy ? (
        ...
        <SendIcon aria-hidden className="size-[19px]" strokeWidth={1.5} />
      )}
    </button>
  );

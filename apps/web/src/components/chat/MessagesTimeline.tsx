...
function AssistantMessageMeta({
  ...
  return (
    ...
    >
      ...
      />
      {forkTurnId !== undefined && <ForkFromMessageButton messageId={message.id} />}
      {!message.streaming && (
        <Tooltip>
          <TooltipTrigger render={<p className="text-muted-foreground text-xs tabular-nums" />}>
 ...
 *  image) cannot take a currentColor tint, so failed rows using them get a
 *  trailing x instead. */
function toolIconAcceptsTint(
  iconName: WorkEntryIconName,
  toolIcon: ToolActivityIcon | undefined,
): boolean {
  return toolIcon === undefined && iconName !== "computer" && iconName !== "lecturn";
}

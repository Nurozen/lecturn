...
import { type EnvironmentConnectionPresentation } from "@lecturn/client-runtime/connection";
import { wasBootstrapThreadDeleted } from "@lecturn/client-runtime/errors";
import { type CodexArtifactTemplate } from "@lecturn/client-runtime/codex-artifact-templates";
import { effectiveSnoozed, threadWokeAt } from "@lecturn/client-runtime/state/thread-settled";
import {
...
import { projectScriptCwd, projectScriptRuntimeEnv } from "@lecturn/shared/projectScripts";
import { truncate } from "@lecturn/shared/String";
import { resolveThreadReferenceCopyTarget } from "@lecturn/shared/threadReference";
import {
  getTerminalLabel,
  nextTerminalId,
  resolveTerminalSessionLabel,
} from "@lecturn/shared/terminalLabels";
...
import { buildTemporaryWorktreeBranchName } from "@lecturn/shared/git";
...
import { resolveProviderSkillsForCwd } from "@lecturn/client-runtime/providerSkills";
...
function ChatViewContent(props: ChatViewProps) {
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `lecturn:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  // the sent message can stay near the top. Lecturn only owns streaming adjustments
  ...
  const parkedThreadBannerItem = useMemo<ComposerBannerStackItem | null>(() => {
    if (!activeThreadSnoozed || activeThreadSettled) return null;
    return {
      id: `thread-snoozed:${activeThread?.id ?? "unknown"}`,
      variant: "info",
      icon: <AlarmClockIcon />,
      title: "This thread is snoozed",
      description: "Send a message to wake",
      actions: (
        <Button
          size="xs"
          variant="ghost"
  ...
  const onSend = async (
    ...
    if (activeThreadSettled) {
      toastManager.add({
        type: "info",
        title: "Unsettle this thread to send a message",
        description: "Your draft is kept until you continue the conversation.",
      });
      return;
    }
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Lecturn server to browse pull requests."
      />
  ...
  return (
    <div className="lecturn-chat-surface relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        {supportsPullRequests && activeProject?.stave && activeProjectRef && activeThreadRef ? (
                  ...
                  >
                    {activeThreadSettled ? (
                      <div className="space-y-2 pb-2">
                        <Button
                          className="settled-thread-action h-auto min-h-20 w-full justify-between rounded-2xl px-5 py-6 text-base"
                          variant="outline"
                          disabled={isUnsettling || activeEnvironmentUnavailable}
                          aria-label="Unsettle thread"
                          aria-busy={isUnsettling}
                          title={
                            activeEnvironmentUnavailable
                              ? "Reconnect to unsettle this thread"
                              : "Unsettle to continue this conversation"
                          }
                          onClick={() => void handleUnsettleActiveThread()}
                        >
                          <span className="lecturn-settled-label flex items-center gap-2">
                            <CheckCircle2Icon className="size-5" />
                            Settled
                          </span>
                          <span>{isUnsettling ? "Unsettling…" : "Unsettle"}</span>
                        </Button>
                        <SettledThreadMetadata
                          ...
                          contextWindowLabel={
                            activeContextWindow
                              ? `${formatContextWindowTokens(activeContextWindow.usedTokens)}${activeContextWindow.maxTokens ? ` / ${formatContextWindowTokens(activeContextWindow.maxTokens)}` : " used"}`
                              : null
                          }
                        />
                        {activeEnvironmentUnavailable ? (

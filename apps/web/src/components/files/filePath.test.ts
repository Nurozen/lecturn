...
describe("fileBreadcrumbs", () => {
  it("builds project, directory, and file crumbs", () => {
    expect(fileBreadcrumbs("lecturn", "apps/web/src/main.tsx")).toEqual([
      { label: "lecturn", path: "", kind: "project" },
  ...
  it("starts host paths outside the workspace at the filesystem root", () => {
    expect(fileBreadcrumbs("lecturn", "/tmp/lecturn-cleanup/report.md")).toEqual([
      { label: "tmp", path: "/tmp", kind: "directory" },
      { label: "lecturn-cleanup", path: "/tmp/lecturn-cleanup", kind: "directory" },
      { label: "report.md", path: "/tmp/lecturn-cleanup/report.md", kind: "file" },
    ]);
    expect(fileBreadcrumbs("lecturn", "C:\\Temp\\report.md")).toEqual([
      { label: "C:", path: "C:", kind: "directory" },
      { label: "Temp", path: "C:\\Temp", kind: "directory" },
      { label: "report.md", path: "C:\\Temp\\report.md", kind: "file" },
    ]);
    expect(fileBreadcrumbs("lecturn", "\\\\server\\share\\report.md").map((c) => c.path)).toEqual([
      "\\\\server",
      "\\\\server\\share",

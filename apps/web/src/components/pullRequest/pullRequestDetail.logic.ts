...
export function pullRequestCheckoutCommand(
  ...
  switch (provider) {
    ...
    case "bitbucket": {
      ...
      return `git clone --single-branch --branch ${headBranch} https://bitbucket.org/${headRepositoryNameWithOwner}.git lecturn-pr-${number}`;
    }
    case "unknown":
      return null;
  }
...
const pullRequestDetailSnapshotKey = (
  ...
  `lecturn.pullRequests.detail:${environmentId}:${reference.projectId}:${reference.host?.toLowerCase() ?? ""}:${reference.repository}#${reference.number}`;

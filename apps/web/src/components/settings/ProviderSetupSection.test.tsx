...
describe("Antigravity setup", () => {
  ...
  it.each([true, false])(
    "blocks managed changes for a custom path (installed=%s), then restores them when cleared",
    async (installed) => {
      setup.auth = authState({ phase: "idle", flowId: null, authorizationUrl: null });
      setup.installation = {
        ...setup.installation!,
      ...
      const custom = { provider: { ...provider, installed }, binaryPath: "/custom/antigravity" };
      const view = renderSetup(custom);
      expect(() => click(view, "Reinstall Antigravity")).toThrow("Button is disabled");
      expect(() => click(view, "Remove downloaded runtime")).toThrow("Button is disabled");
      expect(setup.startInstall).not.toHaveBeenCalled();
      expect(setup.removeInstall).not.toHaveBeenCalled();
      expect(setup.confirm).not.toHaveBeenCalled();

      click(renderSetup({ ...custom, binaryPath: "" }), "Reinstall Antigravity");
      await flushPromises();
      expect(setup.startInstall).toHaveBeenCalledWith({ environmentId, input: { instanceId } });
      setup.confirm.mockResolvedValue(true);
      click(renderSetup({ ...custom, binaryPath: "" }), "Remove downloaded runtime");
      await flushPromises();
      expect(setup.removeInstall).toHaveBeenCalledWith({ environmentId, input: { instanceId } });
    },
  );

  it("keeps shared installation progress and cancellation with a custom path", async () => {
    setup.installation = {
      ...setup.installation!,
      phase: "downloading",
    ...
    const view = renderSetup({ binaryPath: "/custom/antigravity" });
    expect(
      visitElements(view, (element) => element.props.children === "Downloading 1.0 MB."),
    ).not.toBeNull();
    click(view, "Cancel installation");
    await flushPromises();
    expect(setup.cancelInstall).toHaveBeenCalledWith({
      environmentId,
      input: { instanceId, operationId: "shared-op" },
    });
  });

...
describe("hostedPairing", () => {
  ...
  it("reads hosted pairing host and query token parameters", () => {
    const url = new URL(
      "https://lecturn.cloudgatherer.net/pair?host=100.64.1.2:3773&token=ABCD1234",
    );
  ...
  it("prefers hash tokens so generated hosted links do not put credentials in search params", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://preview.lecturn.cloudgatherer.net");
    ...
    expect(url.origin).toBe("https://preview.lecturn.cloudgatherer.net");
  ...
  it("builds hosted channel selection URLs through the configured router origin", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://lecturn.cloudgatherer.net");
    ...
    expect(url.origin).toBe("https://lecturn.cloudgatherer.net");
    expect(url.pathname).toBe("/__lecturn/channel");
  ...
  it("ignores incomplete hosted pairing requests", () => {
    expect(
      hasHostedPairingRequest(
        new URL("https://lecturn.cloudgatherer.net/pair?host=backend.example.com"),
      ),
    ).toBe(false);
    expect(
      hasHostedPairingRequest(new URL("https://lecturn.cloudgatherer.net/pair?token=ABCD1234")),
    ).toBe(false);
  });
  ...
  it("detects the hosted static app only when no backend URL is configured", () => {
    ...
    expect(isHostedStaticApp(new URL("https://preview.lecturn.cloudgatherer.net/"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://preview.lecturn.cloudgatherer.net/pair"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://backend.example.com/"))).toBe(false);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://preview.lecturn.cloudgatherer.net/"))).toBe(false);
  });
  ...
  it("detects hosted channel aliases as static apps", () => {
    ...
    expect(isHostedStaticApp(new URL("https://nightly.lecturn.cloudgatherer.net/"))).toBe(true);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://nightly.lecturn.cloudgatherer.net/"))).toBe(false);
  });
});

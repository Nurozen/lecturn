...
import { HostProcessUserId } from "@lecturn/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Lecturn could not process the desktop app request."),
        );
      });
    });
...
>()("@lecturn/desktop/app/DesktopAppActivation") {}

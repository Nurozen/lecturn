...
function getGnomeKeyringRemediationMessage(): string {
  return "Lecturn could not access GNOME Keyring to save this environment credential. Install and start GNOME Keyring, then restart Lecturn.";
}

function getKWalletRemediationMessage(): string {
  return "Lecturn could not access KWallet to save this environment credential. Enable the KDE wallet subsystem in System Settings, then restart Lecturn.";
}

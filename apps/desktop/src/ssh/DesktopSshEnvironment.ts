...
import * as SshAuth from "@lecturn/ssh/auth";
import { resolveSshTarget } from "@lecturn/ssh/command";
import { discoverSshHosts } from "@lecturn/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@lecturn/ssh/errors";
import * as SshTunnel from "@lecturn/ssh/tunnel";
...
>()("@lecturn/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteLecturnRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Lecturn window is not available for SSH authentication.";
      break;

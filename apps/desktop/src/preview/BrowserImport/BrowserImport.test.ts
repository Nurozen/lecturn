...
const withImporter = Effect.fnUntraced(function* () {
  ...
  const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "lecturn-import-" });
  ...
  const importer = yield* BrowserImport.BrowserImport.pipe(
    Effect.provide(
      BrowserImport.layer.pipe(
        ...
        Layer.provide(Layer.succeed(HostProcessExecutablePath, "/Applications/Lecturn.app")),
    ...
    Effect.gen(function* () {
      ...
      const error = yield* importer
        .importCookies({
          ...
          scope: "persist:lecturn-preview-test",
          persistent: true,
        })
        .pipe(Effect.flip);

...
const withSourceHome = Effect.fnUntraced(function* () {
  ...
  const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "lecturn-sources-" });
...
describe("Helium on Linux", () => {
  it.effect("discovers its profiles and checks the user-data lock", () =>
    run(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "lecturn-helium-linux-" });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, { HOME: home }),
    ...
    run(
      Effect.gen(function* () {
        ...
        const home = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "lecturn-helium-windows-lock-",
        });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, {
            HOME: home,
...
describe("Firefox Snap profiles", () => {
  it.effect("finds Snap profiles with or without profiles.ini and checks their locks", () =>
    run(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "lecturn-firefox-snap-" });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, { HOME: home }),
          Effect.provideService(HostProcessPlatform, "linux"),
        );
    ...
    it.effect(`scans the ${platform} profile location and excludes stale entries`, () =>
      run(
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const home = yield* fileSystem.makeTempDirectoryScoped({
            prefix: `lecturn-firefox-${platform}-`,
          });
  ...
  it.effect("scans for profiles when profiles.ini declares only ones without cookies", () =>
    run(
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const home = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "lecturn-firefox-stale-ini-",
        });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, { HOME: home }),
          Effect.provideService(HostProcessPlatform, "darwin"),
...
describe("isSourceRunning for Firefox", () => {
  it.effect("finds the lock inside the profile, not at the root", () =>
    run(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const home = yield* fileSystem.makeTempDirectoryScoped({ prefix: "lecturn-firefox-" });
        const context = yield* sourcePathContext.pipe(
          Effect.provideService(HostProcessEnvironment, { HOME: home }),
          Effect.provideService(HostProcessPlatform, "darwin"),
        );
  ...
  it.effect("reports not-held when no interpreter can run the fcntl probe", () =>
    run(
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const directory = yield* fileSystem.makeTempDirectoryScoped({ prefix: "lecturn-lock-" });

...
describe("BrowserSession", () => {
  ...
  it.effect("derives deterministic partitions and memoizes sessions", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(partition, "persist:lecturn-preview-f051bb2c68cb7b2fe969");
  ...
  it.effect("keeps scopes that differ only by a lone surrogate in separate partitions", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(
        ...
        "persist:lecturn-preview-f051bb2c68cb7b2fe969",
      );
    }).pipe(Effect.provide(layer)),
  );
  ...
  it.effect("keeps legacy defaults disjoint from nondefault profile partitions", () =>
    Effect.gen(function* () {
      ...
      assert.strictEqual(legacyDefault, "persist:lecturn-preview-78f0be89237d77f7a70e");
      assert.strictEqual(nondefaultProfile, "persist:lecturn-preview-profile-78f0be89237d77f7a70e");

...
const OVERLAY_ATTRIBUTE = "data-lecturn-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--lecturn-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--lecturn-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--lecturn-radius": theme.radius,
    "--lecturn-background": theme.background,
    "--lecturn-foreground": theme.foreground,
    "--lecturn-popover": theme.popover,
    "--lecturn-popover-foreground": theme.popoverForeground,
    "--lecturn-primary": theme.primary,
    "--lecturn-primary-foreground": theme.primaryForeground,
    "--lecturn-muted": theme.muted,
    "--lecturn-muted-foreground": theme.mutedForeground,
    "--lecturn-accent": theme.accent,
    "--lecturn-accent-foreground": theme.accentForeground,
    "--lecturn-border": theme.border,
    "--lecturn-input": theme.input,
    "--lecturn-ring": theme.ring,
    "--lecturn-font-sans": theme.fontSans,
    "--lecturn-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  cursorStyle.textContent = `html[data-lecturn-annotation-tool] body, html[data-lecturn-annotation-tool] body * { cursor: crosshair !important; } [${OVERLAY_ATTRIBUTE}], [${OVERLAY_ATTRIBUTE}] * { cursor: default !important; } [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-inner-spin-button, [${OVERLAY_ATTRIBUTE}] input[type=number]::-webkit-outer-spin-button { appearance:none; margin:0; }`;
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-lecturn-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--lecturn-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-lecturn-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;

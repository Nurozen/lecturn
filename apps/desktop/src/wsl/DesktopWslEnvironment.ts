...
import { buildRemoteNodeEnvScript } from "@lecturn/ssh/tunnel";
...
>()("@lecturn/desktop/wsl/DesktopWslEnvironment") {}
...
// that install wrote.
const WSL_RUNTIME_READY_MARKER = ".lecturn-wsl-runtime-ready";
const WSL_RUNTIME_SELECTED_MARKER = ".lecturn-wsl-runtime-selected";
...
export const buildWslRuntimeInstallScript = (
  ...
  return [
    ...
    '    [ -f "${candidate%/*}/lecturn-wsl-node-pty.json" ] || continue',
    "    return 0",
    "  done",
    "  return 1",
    "}",
...
const marker = path.join(prebuildDir, "lecturn-wsl-node-pty.json");
...
const NODE_PTY_BUILD_SCRIPT = (linuxServerDir: string) =>
  [
    ...
    `printf '{"arch":"%s","modules":"%s","nodePtyVersion":"%s"}\\n' "$arch" "$modules" "$node_pty_version" > "$prebuild_dir/lecturn-wsl-node-pty.json"`,

...
describe("DesktopBackendConfiguration", () => {
  ...
  it.effect("resolveWsl forwards the T3CODE_STAVE kill switch across the wsl.exe boundary", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3-desktop-backend-config-test-",
      ...
      const previousWslEnv = process.env.WSLENV;
      const previousStave = process.env.T3CODE_STAVE;
      try {
        delete process.env.WSLENV;
        process.env.T3CODE_STAVE = "false";

        yield* Effect.gen(function* () {
          const configuration = yield* DesktopBackendConfiguration.DesktopBackendConfiguration;
          const config = yield* configuration.resolveWsl({ port: 5050, distro: null });

          assert.equal(config.env.T3CODE_STAVE, "false");
          assert.include(config.env.WSLENV?.split(":") ?? [], "T3CODE_STAVE");
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
              Layer.provideMerge(serverExposureLayer),
        ...
        restoreEnv("WSLENV", previousWslEnv);
        restoreEnv("T3CODE_STAVE", previousStave);
      }
    }).pipe(Effect.scoped, Effect.provide(NodeServices.layer)),
  );

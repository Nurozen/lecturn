...
const remoteDebuggingPort = process.env.LECTURN_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line lecturn/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.LECTURN_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.LECTURN_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--lecturn-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--lecturn-dev-root=${desktopDir}`, "dist-electron/main.cjs"];

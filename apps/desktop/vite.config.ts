...
const shouldLaunchElectronAfterPack = process.env.LECTURN_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["lecturn#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && cross-env LECTURN_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["lecturn#build"],
        cache: false,
      },
      "dev:bundle": {
